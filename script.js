...
form.addEventListener("submit", async (e) => {
    ...
    try {
        const res = await fetch("http://localhost:5678/webhook/a7f4ff3f-3639-4955-a1b3-98588a5efd5d", {
            method: "POST",
        ...
        if (res.ok) {
            ...
            console.error("n8n response:", res.status, text);
        }
    } catch (err) {
        // network error or CORS error etc.
        responseMsg.textContent = "⚠️ Error connecting to server.";
        console.error("Fetch error:", err);
    }
});

// Prevent default submit and log entered data
...
document.getElementById('register-form').addEventListener('submit', function(e) {
    ...
    console.log("Registration Data:", { name, email, roll, password });
